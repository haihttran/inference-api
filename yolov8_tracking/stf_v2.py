...
def run(
    ...
    # rect = (90,29,495,1392) # Area A
    rect = (820,410,495,690) # Area B
    # rect = (1180,440,495,690) # Area C   # layton 1440 (335,29,983,1392)  pilot (1194,20,927,1411)
    # rect = (1180,385,150,805)
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        for i, det in enumerate(p):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if 'rl' == direction:
                            objbox_front = bboxes[0]
                            objbox_rear = bboxes[2]
                            countingbox_leading_edge = counting_rect[2]
                            countingbox_rear_edge = counting_rect[0]
                        ...
                        if cls == 0 and id not in dict_initial_animal_xyxy :
                            ...
                            pass
                        if cls == 0 and 'rl' == direction:
                            obj_initial_xyxy_front = dict_initial_animal_xyxy[id][0]
                        ...
                        # if cls == 0:  # and det_width < 200 and det_height < 200:
                        if cls == 0 and det_width > 100 and det_height > 100:
                            ...
                            if temp > 0.2:
                                ...
                                if 'rl' == direction:
                                    obj_initial_twenty_in_front = dict_initial_animal_twenty_in_xyxy[id][0]

                            #animal walking forward
                            if temp > 0.5 and obj_initial_xyxy_front - countingbox_leading_edge >  (countingbox_rear_edge - countingbox_leading_edge) * 0.9 : # and objbox_rear > countingbox_leading_edge : # and obj_initial_twenty_in_rear - countingbox_leading_edge < 200 :
                                animal_counter[id] += 1
                                add_animal_to_indi_count(id, dict_animal_indi_count, animal_counter)

                            #animal going backwards dir
                            elif temp > 0.9 and obj_initial_xyxy_front - countingbox_rear_edge < 50 : #and obj_initial_twenty_in_front - countingbox_rear_edge > 0 :
                                animal_counter_minuser[id] += 1
                                add_animal_to_indi_count_minuser(id, dict_animal_indi_count,
                                                                 animal_counter_minuser)
                                print("animal going backwards")

                            #animal that went backward and is going forward again
                            elif temp > 0.3 and id in animal_counter_minuser and objbox_front - countingbox_rear_edge < 0:
                                animal_counter_minuser.pop(id)
                                dict_animal_indi_count.pop(id)

                            #animal going forward that's going backwards now
                            elif temp < 0.1 and id in animal_counter and objbox_rear - countingbox_leading_edge > 0 :
                                animal_counter.pop(id)
                                print("animal going forwards that is backwards")
                                try:
                                    dict_animal_indi_count.pop(id)
                                except Exception as e:
                                    print(e)
 
                            if temp == 0 and id in animal_counter:
                                pass
                                # animal_counter.pop(id)
                                # new_id = id + 60000
                                # animal_counter[new_id] += 1
                                #add_animal_to_indi_count(new_id, dict_animal_indi_count, animal_counter)
                                # print("giving new id")
                                #delete track is animal is more than 100px away from right edge and was counted in positive count
                            if objbox_front - countingbox_rear_edge > 200:
                                pass
                                #deepsort.remove_id(id)
                                #dict_initial_animal_xyxy.pop(id)
                            #don't track if animal is more than 200px from leading edge
                            #if countingbox_leading_edge - objbox_rear > 200:
                                    #remove id from deep_sort
                            #        deepsort.remove_id(id)
                            ...
                            global_label_left = f'Going left: {print_animal_count}' # {names[c]} {conf:.2f}'
                            global_label_right = f'Going right: {print_animal_count_minuser}' # {names[c]} {conf:.2f}'

                            annotator.box_label(counting_rect, None, color=colors(10, True))

                        if cls == 0 and id in dict_animal_indi_count and temp > 0.2 :
                            ...
                            annotator.box_label(bboxes,'', color=(255,0,0))
                        else:
                            pass
            ...
            annotator.box_label((4, 150, 2, 2), global_label_right, color=colors(10, True))
            annotator.box_label((4, 0, 2, 2), global_label_left, color=colors(0, True))
            ...
            if save_vid:
                if vid_path != save_path:  # new video
                    vid_path = save_path
                    resizedFrame = rescale_frame(im0)
                    if isinstance(vid_writer, cv2.VideoWriter):
                        vid_writer.release()  # release previous video writer

                    fps, w, h = 25, resizedFrame.shape[1], resizedFrame.shape[0]
                    save_path = str(Path(save_path).with_suffix('.mp4')) 
                    vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
                    #vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc('V','P','8','0'), fps, (w, h))
                resizedFrame = rescale_frame(im0)
                vid_writer.write(resizedFrame)
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=Path,default='/home/rama/Documents/code-repo/github/yolov8_tracking/weights/yolo/yolov8l_pigs_24052023.pt',help='model.pt path(s)')
    parser.add_argument('--reid-weights', type=Path, default=WEIGHTS / 'reid/osnet_x1_0_imagenet.pt')
